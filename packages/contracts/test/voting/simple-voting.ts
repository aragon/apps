...
import {defaultAbiCoder, BytesLike, isAddress} from 'ethers/lib/utils';
...
const EVENTS = {
  ...
  CAST_VOTE: 'CastVote',
  PROCESS_EXECUTED: 'ProcessExecuted',
};
...
function createProposal({
  ...
  const proposal = {
    ...
    additionalArguments: defaultAbiCoder.encode(
      ['string', 'bool', 'bool'],
      [description || '0x', executeIfDecided || false, castVote || false]
    ),
  };
...
function createVoteData(supports: boolean, executeIfDecided: boolean) {
  return defaultAbiCoder.encode(['bool', 'bool'], [supports, executeIfDecided]);
}
...
describe('Voting: SimpleVoting', function () {
  ...
  describe('changeConfig: ', async () => {
    ...
    it('reverts if wrong config is set', async () => {
      await expect(voting.changeVoteConfig(1, 2)).to.be.revertedWith(
        ERRORS.ERROR_CHANGE_SUPPORT_PCTS
      );

      await expect(voting.changeVoteConfig(pct16(1000), 1)).to.be.revertedWith(
        ERRORS.ERROR_CHANGE_SUPPORT_TOO_BIG
      );
    });
  ...
  describe('StartVote', async () => {
    ...
    it('reverts if allowed action is not passed through proposal', async () => {
      ...
      await expect(voting.start(proposal)).to.be.revertedWith(
        ERRORS.NOT_ALLOWED_ACTION
      );
    });
  ...
  describe('Vote + Execute:', async () => {
    ...
    it('executes the vote immediatelly while final yea is given', async () => {
      ...
      expect(await voting.vote(1, createVoteData(true, true)))
        .to.emit(voting, EVENTS.PROCESS_EXECUTED)
        .withArgs(
          ownerAddress,
          [
            // execution struct
      ...
      await expect(voting.execute(1)).to.be.revertedWith(
        ERRORS.ERROR_EXECUTION_STATE_WRONG
      );
    });
